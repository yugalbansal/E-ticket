...
const EventDetails = () => {
  ...
  // ✅ Use event from navigation state
  const event = location.state?.event;

  if (!event) {
    return <div className="text-center text-red-500 text-xl font-bold mt-10">No event found!</div>;
  }
  ...
  const handlePaymentWithTelos = async () => {
    ...
    try {
      ...
      const txHash = await makePaymentWithTelos(amount, event.organizer?.address || '');
      console.log('Transaction hash:', txHash);
      handlePaymentSuccess();
    } catch (error: any) {
      console.error('Payment failed:', error);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="relative h-96">
            ...
            <div className="absolute top-4 right-4 space-x-2">
              <button onClick={handleShare} className="p-2 bg-white/80 backdrop-blur-sm rounded-full hover:bg-white">
                <Share2 className="w-5 h-5 text-gray-700" />
              </button>
              <button className="p-2 bg-white/80 backdrop-blur-sm rounded-full hover:bg-white">
                <Heart className="w-5 h-5 text-red-500" />
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                ...
                <div className="space-y-2">
                  {Object.entries(event.price).map(([type, price]) => (
                    <button
                      ...
                      onClick={() => setSelectedTicket(type)}
                      className={`w-full flex items-center justify-between p-4 rounded-lg border ${
                        selectedTicket === type ? 'border-indigo-600 bg-indigo-50' : 'border-gray-200 hover:border-indigo-600'
                      }`}
                    >
                      <div className="flex items-center">
                        {type === 'vip' ? <Crown className="w-5 h-5 mr-2 text-indigo-600" /> : type === 'group' ? <Users className="w-5 h-5 mr-2 text-indigo-600" /> : <Ticket className="w-5 h-5 mr-2 text-indigo-600" />}
                        <span className="capitalize">{type} Ticket</span>
                      </div>
                      <span className="font-semibold">${price.toFixed(2)}</span>
                    </button>
              ...
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">Quantity</h3>
                <div className="flex items-center space-x-4">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2 rounded-md border border-gray-200 hover:border-indigo-600">-</button>
                  <span className="text-xl font-semibold">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="p-2 rounded-md border border-gray-200 hover:border-indigo-600">+</button>
                </div>

                <div className="mt-8">
                  <button onClick={handlePaymentWithTelos} disabled={isProcessing} className="w-full py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 disabled:opacity-50">Pay with Telos</button>
                  <button onClick={handlePaymentWithRazorpay} disabled={isProcessing} className="w-full py-3 bg-white text-gray-700 border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50">Pay with Razorpay</button>
                </div>
              </div>
            </div>
          </div>
