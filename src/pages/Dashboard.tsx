...
const Dashboard = () => {
  ...
  // ✅ Debugging: Log user and tickets when the component renders
  useEffect(() => {
  ...
  // ✅ Check if user exists and has tickets
  const renderTickets = () => {
    if (!user) {
      return (
        <div className="text-center py-12">
          <h3 className="text-xl font-medium text-gray-900">You are not logged in!</h3>
          <p className="mt-2 text-sm text-gray-500">Please log in to view your tickets.</p>
          <div className="mt-6">
            <Link
              to="/login"
              className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700"
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              <div className="space-y-1">
                <button
                  onClick={() => setActiveTab('tickets')}
                  className={`w-full flex items-center px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                    activeTab === 'tickets' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  <Ticket className="w-5 h-5 mr-3" />
                  My Tickets
